...
class DBAuth{
	...
	*/
	public function login($nom,$mdp){
		$user=$this->db->prepare('SELECT * FROM user WHERE  pseudo=? ',[$nom],null,true);
		if($user){
			if($user->motdepass === md5($mdp)){
				$_SESSION['auth']=$user->id;
				$_SESSION['nom']=$user->nom;
				$_SESSION['mdp']=$user->motdepass;
	...
	public function adminLogged(){
		if($this->logged()  && ($_SESSION['pseudo']== "admin")  && ($_SESSION['mdp']==md5("admin"))){
				return true ;
		}
		return false;
	}
